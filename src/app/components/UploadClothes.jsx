...
export default function UploadClothes() {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-8 max-w-2xl mx-auto mt-10 border border-gray-100">
      <h2 className="text-3xl font-extrabold text-gray-800 mb-6 text-center tracking-tight">Upload Your Clothes</h2>
      ...
      <div className="flex flex-col items-center gap-4">
        <label className="w-full cursor-pointer flex flex-col items-center py-6 px-4 border-2 border-dashed border-blue-300 rounded-xl bg-blue-50 hover:bg-blue-100 transition">
          <span className="text-blue-700 font-medium mb-2">Select images to upload</span>
          <input
            type="file"
            multiple
            accept="image/*"
        ...
        {previews.length > 0 && (
          <div className="w-full mt-4">
            <h3 className="text-base font-semibold text-gray-700 mb-2">Selected Images</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {previews.map((src, idx) => (
                <div key={idx} className="relative group bg-gray-50 rounded-xl shadow p-2 flex flex-col items-center transition hover:shadow-lg">
                  <img
                    src={src}
                    alt={`preview-${idx}`}
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            {uploaded.map((url, idx) => (
              <div key={idx} className="bg-gray-50 rounded-xl shadow p-2 flex flex-col items-center transition hover:shadow-lg">
                <img
                  src={url}
                  alt={`cloth-${idx}`}
